...
void Physics::addEnemy2(AnimatedSprite *player, float x, float y)
{
	//add some things to the sprite fields
	player->setRadius(player->getSpriteType()->getTextureWidth() / 2 * .02f);
	player->setSpawnX(x);
...
void Physics::addEnemy3(AnimatedSprite *player, float x, float y)
{
	//add some things to the sprite fields
	player->setRadius(player->getSpriteType()->getTextureWidth() / 2 * .02f);
	player->setSpawnX(x);
...
{
	//add some things to the sprite fields
	PowerUp *p = (PowerUp *)asteriod;
	if (p->type == 3)
		asteriod->setRadius((100)*.02);
	else
